...
def gen(request):
    ...
    if request.method == 'POST':
        ...
        if 'reset_password' in request.POST:
            context['message'] = request.POST['sel_user']
            for u in User.objects.all():
                if str(u.username)+"314" in request.POST:
                    context['message'] = "Received a reset request for " + str(u.username)
...
def add_user(l_id, user):
    print "add user"
    tmp_l = lists.objects.get(list_id = l_id)
    tmp_u = User.objects.get(username = user)
...
def create_item(user_name, c_list_id, new_item_name, details, quantity):
    ...
    new_item = items(item_name = new_item_name, checked_status = 0, item_details = details, 
               item_quantity = quantity,containing_list_id = c_list_id, item_creator_id = user.id)
    new_item.save()
...
#NEEDS TESTING
def remove_item(i): #i is id or whatever
...
def get_list_tags(l_id):
     lists_tags = []
     tmp_l = lists.objects.get(list_id = l_id)
     lists_tags = tmp_l.list_tags.all()
     return lists_tags
